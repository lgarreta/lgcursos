...
def createCmmForOS (programName, inputsFile, resultsFile):
    ...
    if platform == "linux" or platform == "linux2":
        cmm = "java -cp .:sqlite-jdbc-3.7.2.jar %s < %s > %s " % (programName, inputsFile, resultsFile)
        # linux
...
#--------------------------------------------------------------------
#--------------------------------------------------------------------
def executeCmms (programName, inputsFile, resultsFile):
    bdFile = "bd_estudiantes.db"
    if(os.path.exists (bdFile)):
        os.remove (bdFile)
        
    cmm = "sqlite3 bd_estudiantes.db < estudiantes.sql"
    os.system (cmm)
    cmm = "javac *.java"
    os.system (cmm)
...
def probarCmdCases (programName):
    ...
    for inputs, outputs in zip ([inputsCases], [outputsCases]):
        ...
        executeCmms (programName, inputsFile, resultsFile)
...
def compareOutputsWithResults (lineasPrograma, lineasCaso):
    ...
    for i in range (nRespuestas):
        ...
        flagCorrect = "X"
        if any (x in strPrograma for x in ["á","é","í","ó","ú"]):
            flagCorrect = "?"
            contadorBuenas +=1
        elif (strPrograma == strCaso):
            flagCorrect = "/"
            contadorBuenas +=1
