...
# Process the datasets with measures of wearable computing
# All the inputs are in the "inputs" folder
# The output is written into the current working dir

# Steps are indicated in the comments. I coded for clarity
# the third and fourth steps before the second as it gives names to 
# the data (features and activities)
...
dataSubjTrainTest = rbind (cbind (xtrainSubjects, ytrain, xtrain, stringsAsFactors=F), 
						   cbind (xtestSubjects, ytest, xtest, stringsAsFactors=F), stringsAsFactors=F); #view (dataSubjTrainTest)
...
names (dataWithActivities) = c("Activity", "Subject", featuresNames); #view (dataWithActivities)
